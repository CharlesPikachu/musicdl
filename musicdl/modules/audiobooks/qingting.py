...
class QingtingMusicClient(BaseMusicClient):
    ...
    def __init__(self, allowed_search_types: list = None, **kwargs: Unpack[BaseMusicClientKwargs]):
        self.allowed_search_types = list(set(allowed_search_types or QingtingMusicClient.ALLOWED_SEARCH_TYPES))
    ...
    def _parsewithofficialapiv1(self, search_result: dict, request_overrides: dict = None):
        ...
        return song_info
    '''_parsebytrack'''
    def _parsebytrack(self, search_results, song_infos: list = [], request_overrides: dict = None, progress: Progress = None, main_page_no: int = 1):
        # init
        ...
        for search_result_per_track_idx, search_result_per_track in enumerate(search_results['data']['data']):
            # --update progress
            ...
            # --pass invalid items
            if (not isinstance(search_result_per_track, dict)) or (not search_result_per_track.get('id')) or (search_result_per_track.get('type') not in {'program'}): continue
            # --init song info
    ...
    def _parsebyalbum(self, search_results, song_infos: list = [], request_overrides: dict = None, progress: Progress = None, main_page_no: int = 1):
        ...
        for search_result in search_results['data']['data']:
            ...
            for page_num_idx, page_num in enumerate(range(1, num_pages + 1)):
                if page_num_idx > 0: progress.advance(download_album_pid, 1); progress.update(download_album_pid, description=f"{self.source}.p{main_page_no}._parsebyalbum >>> ({page_num_idx}/{num_pages}) pages downloaded in album {album_id}")
            ...
            progress.advance(download_album_pid, 1); progress.update(download_album_pid, description=f"{self.source}.p{main_page_no}._parsebyalbum >>> ({page_num_idx+1}/{num_pages}) pages downloaded in album {album_id}")
