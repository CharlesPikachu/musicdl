        ...
        try:
            ...
            for search_result in search_results:
                ...
                if (resp is not None) and (resp.status_code in [200]):
                    ...
                    track_info = download_result.get('trackInfo', {'playHqSize': ''}) or {'playHqSize': ''}
                    qualities = [
                    ...
                    for quality in qualities:
                        file_size, download_url = str(track_info.get(quality[0], '0')).strip() or '0', track_info.get(quality[1], '')
                        if download_url:
                    ...
                    if download_url:
                        ...
                        duration = int(str(track_info.get('duration', '0')).strip() or '0')
                ...
                if not download_result or not download_url:
                    ...
                    track_info = download_result.get('trackInfo', {'playUrlList': []}) or {'playUrlList': []}
                    for encrypted_url in sorted(track_info.get('playUrlList', []), key=lambda x: int(x['fileSize']), reverse=True):
                        download_url = self._decrypturl(encrypted_url.get('url', ''))
                        if download_url:
                            download_url_status = AudioLinkTester(headers=self.default_download_headers, cookies=self.default_cookies).probe(download_url, request_overrides)
                            if download_url_status['ok']: break
                    if download_url:
                        file_size = str(encrypted_url.get('fileSize', '0')).strip() or '0'
                        file_size = f'{round(int(file_size) / 1024 / 1024, 2)} MB'
                        ext = download_url.split('.')[-1].split('?')[0]
                        duration = int(str(track_info.get('duration', '0')).strip() or '0')
                        duration = seconds2hms(duration)
                ...
                if (not download_result or not download_url) and ('n' in search_result):
                    ...
                    if download_url:
                        try:
                            download_result_suppl = probesongurl(download_url, headers=self.default_download_headers, cookies=self.default_cookies, request_overrides=request_overrides)
                        except:
                            download_result_suppl = {'download_url': download_url, 'file_size': '0.00 MB', 'ext': download_url.split('.')[-1].split('?')[0]}
                        if 'ext' not in download_result_suppl or download_result_suppl['ext'] == 'NULL':
                            download_result_suppl['ext'] = download_url.split('.')[-1].split('?')[0]
                        download_result['suppl'] = download_result_suppl
                        download_url_status = AudioLinkTester(headers=self.default_download_headers, cookies=self.default_cookies).probe(download_url, request_overrides)
